  ...
  if msg.startswith("$"):
    ...
    for date in dates:
      ...
      if not ind_arr:
        ...
        for j in ind_arr:
          result = result + user_input.upper() + str(options_chain['calls']['Strike'][j]) + "C" + " expires on " + date + " it currently has a bid/ask spread of " + str(options_chain['calls']['Bid'][j]) + " : " + str(options_chain['calls']['Ask'][j]) + "." + " The current change is: " + str(options_chain['calls']['Change'][j]) + " (" + str(options_chain['calls']['% Change'][j]) + ")." +  " OI: " + str(options_chain['calls']['Open Interest'][j]) + ", Vol: " + str(options_chain['calls']['Volume'][j]) + ", IV: " + str(options_chain['calls']['Implied Volatility'][j]) + "." + "\n"
      ...
      if not ind_arr2:
        ...
        for t in ind_arr2:
          result = result + user_input.upper() + str(options_chain['puts']['Strike'][t]) + "P" + " expires on " + date + " it currently has a bid/ask spread of " + str(options_chain['puts']['Bid'][t]) + " : " + str(options_chain['puts']['Ask'][t]) + "." + " The current change is: " + str(options_chain['puts']['Change'][t]) + " (" + str(options_chain['puts']['% Change'][t]) + ")." +  " OI: " + str(options_chain['puts']['Open Interest'][t]) + ", Vol: " + str(options_chain['puts']['Volume'][t]) + ", IV: " + str(options_chain['puts']['Implied Volatility'][t]) + "." + "\n"
    ...
    result += "The current stock price for " + user_input.upper()  + " is " + str(round(current_price, 2))
